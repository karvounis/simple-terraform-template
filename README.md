...
It scans cloud infrastructure provisioned using Terraform, Cloudformation, Kubernetes, Serverless or ARM Templates and  
...
Just empty the contents of this file and you are good to go!

## Automation

You can use all the above makefile targets as precommit hooks.  
The only dependency is the [pre-commit](https://pre-commit.com/) tool.  
You can also install it so that it runs automatically on every git commit.  
The configuration file is [here](./.pre-commit-config.yaml).
